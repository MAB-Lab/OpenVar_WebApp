			...
			if (data.outcome == 'success') {
				...
				if (data.hotspots_allnulls == 'False') {
					...
					var bar_hotspot = new Chart(hotspot_bar, {
						...
						options: { legend: {display: false}, layout: { padding: {left:0, right:0, top:0, bottom:0} }, scales:{ xAxes:[{stacked: false}], yAxes: [{stacked: true}]}, title: { display: true, text:'Mutational hotspots on alternative proteins', fontSize:16}, tooltips: { callbacks: { afterLabel: function(t, d){ return 'Average impact of SNPs: ' + Math.round(d.datasets[t.datasetIndex].average_impact[t.index]*100)/100 } }} },
					});
				} else {
